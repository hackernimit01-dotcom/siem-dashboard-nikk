...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (themeToggle && themeForm) {
        themeToggle.addEventListener('change', function(e) {
            const theme = e.target.checked ? 'dark' : 'light';
            const input = themeForm.querySelector('input[name="theme"]');
            if (input) {
                input.value = theme;
            }
            themeForm.submit();
    ...
    if (customizeForm) {
        customizeForm.addEventListener('change', function() {
            ...
            const showThreatMapCheckbox = customizeForm.querySelector('[name="showThreatMap"]');
            const showSeverityChartCheckbox = customizeForm.querySelector('[name="showSeverityChart"]');
            const eventsTable = document.querySelector('.data-table-container');

            if (threatMap && showThreatMapCheckbox) {
                const threatMapContainer = threatMap.parentElement?.parentElement;
                if (threatMapContainer) {
                    threatMapContainer.style.display = showThreatMapCheckbox.checked ? '' : 'none';
                }
            }

            if (showSeverityChartCheckbox) {
                const severityChart = document.getElementById('severityChart');
                if (severityChart) {
                    const severityChartContainer = severityChart.parentElement?.parentElement?.parentElement;
                    if (severityChartContainer) {
                        severityChartContainer.style.display = showSeverityChartCheckbox.checked ? '' : 'none';
                    }
                }
            }

            if (eventsTable) {
                const eventsTableCheckbox = customizeForm.querySelector('[name="showEventsTable"]');
                if (eventsTableCheckbox) {
                ...
                searchInput.addEventListener('input', () => {
                    ...
                    rows.forEach(row => {
                        ...
                        if (detailsId) {
                            ...
                            if (detailsRow) {
                                detailsRow.style.display = visible && row.classList.contains('expanded') ? 'table-row' : 'none';
                            }
                        }
                    });
                });
...
function initializeMaps() {
    // Skip the fallback map if Leaflet is already loaded by a page-specific script.
